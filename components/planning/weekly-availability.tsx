    ...
    <Card className="mb-8 p-6">
      <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
        <div>
          ...
          <h2 className="mt-2 text-2xl font-black text-court-900">Réservations</h2>
          <p className="mt-3 text-sm leading-6 text-ink-500">
            Semaine du {week.label}. Les annulations exceptionnelles et les créneaux réservables sont pris en compte.
          </p>
        </div>

        <div className="grid gap-3">
          {loading ? <p className="rounded-lg bg-court-50 p-4 text-sm font-semibold text-ink-500">Chargement des créneaux...</p> : null}
          ...
          {items.map((item) => (
            ...
            >
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-ink-500">{item.gymnase}</p>
                {item.is_cancelled ? (
              ...
              <p
                className={
                  item.is_cancelled
                    ? "rounded-lg bg-red-50 px-3 py-2 text-center text-sm font-black text-red-700"
                    : item.reservation_active
                      ? "rounded-lg bg-white px-3 py-2 text-center text-sm font-black text-court-700"
                      : "rounded-lg bg-white px-3 py-2 text-center text-sm font-black text-ink-500"
                }
              >
                {item.is_cancelled ? "Annulé" : item.reservation_active ? "Réservable" : "Sans réservation"}
              </p>
            </div>
          ))}
        </div>
